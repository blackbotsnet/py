                ...
                if not B:res_box.markdown(f":blue[Dao: ]:green[*Enter a valid URL before running.*]")
                        ...
                        try:
                                ...
                                if G.status_code==200:
                                        M=BeautifulSoup(G.text,_F);D=M.find(_C,{_D:_N})
                                        if D:
                                                ...
                                                with tempfile.NamedTemporaryFile(suffix=_K,delete=_B)as H:A=A.replace('"','');S=gTTS(text=A,lang=_G,slow=_B);S.save(H.name);T=AudioSegment.from_mp3(H.name);U=speedup(T,1.2,150);U.export(_H,format='mp3');autoplay_audio(_H)
                                                for V in O:
                                                        W=''
                                                        for I in V:W+=I.text+_E
                                                        if on:res_box.markdown(f":blue[Dao: ]:green[*{I.text}*]");time.sleep(5)
                                                X=st.button('Next CH.',key=_O,help='Next Chapter',use_container_width=_B)
...
with st.sidebar:
                ...
                with st.spinner('Searching..'):
                        if search_variable:
                                ...
                                if resp.status_code==200:
                                        soup=BeautifulSoup(resp.text,_F);search_result_div=soup.find(_C,{_D:_S})
                                        if search_result_div:
                                                titless=search_result_div.find_all(_C,{_D:_T})
                                                for title in titless:
                                                        title_url=title.a[_L];title_name=title_url.split(_U)[1];title_name=title_name.replace('/','');title_name=title_name.title();img_url=title.img[_M];ch=f"https://daotranslate.us/{title_name}-chapter-1/";ih=ch;st.write(f"[{title_name}]({ih})");st.image(img_url,caption=ih);button_key=title_name+str(random.randint(1,999))
                                                        if ih:
        ...
        with st.expander('Latest Releases'):
                ...
                if resp.status_code==200:
                        soup=BeautifulSoup(resp.text,_F);manga_list_div=soup.find(_C,{_D:_S})
                        if manga_list_div:
                                titles=manga_list_div.find_all(_C,{_D:_T})
                                for title in titles:
                                        title_url=title.a[_L];title_name=title_url.split(_U)[1].replace('/','').title();ih=f"https://daotranslate.us/{title_name}-chapter-1/";st.write(f"[{title_name}]({ih})");img_url=title.img[_M];st.image(img_url,caption=ih,use_column_width='always');button_key=title_name+str(random.randint(1,999))
                                        if ih:
        ...
        with st.expander(_V):
                ...
                if resp.status_code==200:
                        soup=BeautifulSoup(resp.text,_F);manga_links=soup.find_all('a',href=lambda href:href and href.startswith(_W))
                        for link in manga_links:href=link.get(_L);manga_name=href.split(_W)[1];ch=f"{href}";st.caption(manga_name);st.write(f"{ch}");st.divider()
...
tab1,tab2=st.tabs(['Text Based',_V])
with tab1:
        if tab1:
                if ok:
                        with st.spinner(_I):
                                ...
                                if not url:res_box.markdown(f":blue[Dao: ]:green[*Enter a valid URL before running.*]")
                                        ...
                                        try:
                                                ...
                                                if resp.status_code==200:
                                                        soup=BeautifulSoup(resp.text,_F);d=soup.find(_C,{_D:_N})
                                                        if d:
                                                                ...
                                                                with tempfile.NamedTemporaryFile(suffix=_K,delete=_B)as tmp_file:story=story.replace('"','');tts=gTTS(text=story,lang=_G,slow=_B);tts.save(tmp_file.name);audio=AudioSegment.from_mp3(tmp_file.name);new_file=speedup(audio,1.2,150);new_file.export(_H,format='mp3');autoplay_audio(_H)
                                                                for group in groups:
                                                                        group_text=''
                ...
                with st.spinner(_I):
                        ...
                        try:
                                if st.session_state.image_links:
                                        current_image_index=st.session_state.current_image_index;current_image_link=st.session_state.image_links[current_image_index];st.image(current_image_link,use_column_width=_A);next_button_clicked=st.button('Next',key=_O,help='Show next image',use_container_width=_B)
